...
int main()
{
  int M = 640, N = 480, i, j, niter = 200, halo = 255, k;
  float pold[M+2][N+2], pnew[M+2][N+2], plim[M+2][N+2];
  float *data;

  data = (float*)malloc(M * N * sizeof(float));

  pgm_size("serial_image_640x480.pgm", &M, &N);
  pgm_read("serial_image_640x480.pgm", data, M, N);
  ...
  pgm_write("200_serial_image_640x480.pgm", data, M, N);
